...
def main():
    ...
    print(f"横方向のドリフト: {drift:.2f} m") # 落下している姿勢の
    """
    横向きと縦向きを考慮する力
...
def Freefall2D(height_now, verocity_now_z, verocity_now_xy, weight, wind_speed):
    ...
    while height > 0:# x方向の空気抵抗（風を考慮）
        ...
        x_position += verocity_xy * del_time # z方向の重力と空気抵抗
        ...
        z_position_list.append(height)
        x_position_list.append(x_position)
    return time, verocity_z, x_position
    

"""
直線的な落下空気抵抗がある場合は速度が時々刻々と変化するため、加算で求める
height_now 求める時点の高さ
verocity_now_z 現在の速度
weight 落下物の重さ
"""
